...
int main(int argc, const char* argv[]) {
	...
	pthread_t threads[CANT_JUGADORES];
    int socketfd_cliente[CANT_JUGADORES];
    int socket_size;
...
bool chequear_palabra_actual(const list<Casillero>& palabra_actual) {
	for(list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++)
	{
		if(tablero_palabras[casillero->fila][casillero->columna] != VACIO){ return false; }
	}
	return true;
}
...
bool es_ficha_valida_en_palabra(const Casillero& ficha, const list<Casillero>& palabra_actual) {
    ...
    //~ if (tablero_letras[ficha.fila][ficha.columna] != VACIO) {
        //~ return false;
    //~ }
    if (tablero_palabras[ficha.fila][ficha.columna] != VACIO) {
        return false;
    }
